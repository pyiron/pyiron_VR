...
public class LaserGrabber : MonoBehaviour
{
    ...
    private void moveGrabbedObject()
    {
        ...
        if (ctrlMaskName == "AtomLayer")
            SD.atomInfos[attachedObject.GetComponent<AtomID>().ID].m_ctrlTrans.position += newPos - oldPos;
    ...
    private void AttachObject(GameObject grabAbleObject)
    {
        ...
        else if (ctrlMaskName == "AtomLayer")
        {
            ...
            laserLength = (attachedObject.transform.position - transform.position).magnitude;
            SD.currentlyMovedAtomID = attachedObject.GetComponent<AtomID>().ID;
        }
    ...
    private void testForNewStructure()
    {
        bool shouldCreateNewStructure;
        shouldCreateNewStructure = false;
        print("yay");
        for (int i = 0; i < 3 ; i++)
            if (attachedObject.transform.position[i] + attachedObject.transform.localScale[i] > boundingbox.transform.position[i] + boundingbox.transform.localScale[i] || attachedObject.transform.position[i] - attachedObject.transform.localScale[i] < boundingbox.transform.position[i] - boundingbox.transform.localScale[i])
                shouldCreateNewStructure = true;

        print("new Structure? " + shouldCreateNewStructure);
    ...
    private void createNewStructure()
    {
        ...
        newStructure.name = "newStructure"; // should be changed soon!
        // set the parent of the atom which build the new structure to the new structure
        attachedObject.transform.parent = newStructure.transform;
    }
