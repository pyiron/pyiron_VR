...
public class ImportStructure : MonoBehaviour
{
    ...
    {
        loadStructure();
        //print("SD.struccotrltrans : " + SD.structureCtrlTrans.position);
    }
    ...
    private void initAtoms()
    {
        ...
        if (firstImport)
            currentAtom.transform.position = new Vector3(float.Parse(data[0]), float.Parse(data[1]),
                float.Parse(data[2])) - (maxPositions + minPositions) / 2;// + transform.position;
        else
            currentAtom.transform.position = (new Vector3(float.Parse(data[0]), float.Parse(data[1]),
                float.Parse(data[2])) - (maxPositions + minPositions) / 2) * programSettings.size
                + SD.atomInfos[atomCounter].m_ctrlTrans.position + transform.position;
