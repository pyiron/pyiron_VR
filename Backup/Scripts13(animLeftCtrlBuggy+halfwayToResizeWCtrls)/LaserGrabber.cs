...
public class LaserGrabber : MonoBehaviour
{
    ...
    public bool otherCtrlReadyForResize;
    ...
    private void checkHairTrigger()
    {
        ...
        if (Controller.GetHairTriggerDown())
        {
            ...
            else if (otherCtrlReadyForResize)
                    init_resize();
            else
        ...
        if (collidingObject == null)
        {
            ...
            if (!attachedObject)
                ...
                if (Physics.Raycast(trackedObj.transform.position, transform.forward, out hit, laserMaxDistance, ctrlMask))
                {
                    ...
                    if (ctrlMaskName == "AtomLayer")
                        otherCtrl.GetComponent<LaserGrabber>().otherCtrlReadyForResize = true;
        }
    }
    ...
    private void AttachObject(GameObject grabAbleObject)
    {
        ...
        if (ctrlMaskName == "BoundingboxLayer")
        {
            ...
            if (collidingObject)
                ...
                // tell the other controller that this controller is in the state where it can resize the structure
                if (otherCtrlReadyForResize)
                    init_resize();
                else
                    otherCtrl.GetComponent<LaserGrabber>().otherCtrlReadyForResize = true;
                //else
    ...
    private void ReleaseObject()
    {
        ...
        // set the state of the controller to not ready for resize, if it isn't already
        if (otherCtrl.GetComponent<LaserGrabber>().otherCtrlReadyForResize)
            otherCtrl.GetComponent<LaserGrabber>().otherCtrlReadyForResize = false;
